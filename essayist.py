...
SEARCHER_SYSTEM = """Ответь на вопрос, опираясь на веб-поиск. Давай максимум конкретики:
точные числа, даты, имена. Для каждого факта укажи источник (URL/домен из результатов).
Если факта в результатах нет — напиши "не найдено", не выдумывай. Формат, без вступления:
