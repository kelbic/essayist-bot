...
## Параллельный таймер (bot.py)

`run_timer_tick` обрабатывает «созревшие» каналы НЕ последовательно, а через
`asyncio.gather` под `Semaphore(TIMER_CONCURRENCY=3)`:
- **Изоляция**: каждый канал идёт в `_tick_channel` со своим try/except —
  падение одного канала не убивает тик остальных (раньше необработанное
  исключение обрывало весь хвост тика). Ошибка пишется в `last_error` канала.
- **Конкурентность**: семафор ограничивает одновременные генерации. Узкие места
  при росте — общий rate-limit одного Anthropic-ключа (429 ловятся ретраями
  `_Anthropic`, но провоцировать их незачем) и флуд-лимиты Telegram при доставке.
  3 — безопасно; 50 каналов ≈ 21 мин/тик вместо ~60.
- **Расписание не плывёт**: `timer_loop` спит `TICK` минут ПОСЛЕ тика, наложений
  нет. В логе итоговая строка `timer: тик обработал N каналов за X с`. Когда X
  приближается к интервалу тика — поднять `TIMER_CONCURRENCY` (с оглядкой на
  тир Anthropic-ключа).

⚠️ **Общий rate-limit ключа** — единственное настоящее общее узкое место при
росте. Все генерации (ручные + таймер всех каналов) делят лимит одного
Anthropic-ключа. При всплеске параллельных запусков пойдут 429; деградация
мягкая (ретраи в `_Anthropic`, генерации замедлятся, не упадут), но при
устойчивом росте — отдельный ключ под таймер или повышение тира.
